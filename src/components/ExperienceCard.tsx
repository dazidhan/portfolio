  ...
  if (experience.image) {
    return (
      <Image
        ...
        className="h-full w-full object-contain p-8 sm:p-12 lg:p-16 grayscale transition-[filter,scale] duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)] lg:group-hover/media:scale-105 group-hover/media:grayscale-0 motion-reduce:transition-none motion-reduce:group-hover/media:scale-100"
      />
    );
  }
...
export default function ExperienceCard({
  ...
  return (
    ...
    >
      <div className="flex min-h-0 flex-col overflow-y-auto overflow-x-hidden pr-2 px-5 py-4 sm:px-8 sm:py-6 lg:px-10 lg:py-10 [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-full">
        <header>
          ...
          <h3 className="mt-3 max-w-[16ch] text-[clamp(1.2rem,5vw,1.6rem)] font-bold uppercase leading-[1.1] tracking-[-0.03em] sm:mt-4 sm:text-[clamp(1.6rem,3vw,2.5rem)] lg:mt-6">
            {experience.role}
          </h3>

          <p className="mt-2 text-[0.65rem] font-bold uppercase leading-relaxed tracking-[0.08em] sm:mt-3 sm:text-xs lg:mt-4 lg:text-sm">
            {experience.company}
          </p>
        </header>

        {experience.companyDescription && (
          <p className="mt-2 max-w-[55ch] text-[0.6rem] italic leading-[1.3] text-card-foreground/60 sm:mt-3 sm:text-[0.65rem] lg:mt-4 lg:text-xs">
            {experience.companyDescription}
          </p>
        )}

        <p className="mt-2 max-w-[55ch] text-[0.65rem] leading-[1.4] text-card-foreground/75 sm:mt-3 sm:text-[0.7rem] sm:leading-[1.5] lg:mt-4 lg:text-sm lg:leading-[1.6]">
          {experience.summary}
        </p>
        ...
        <div className="mt-3 sm:mt-4 lg:mt-6">
          ...
          <ol className="grid sm:grid-cols-2">
            {experience.highlights.map((highlight, highlightIndex) => (
              ...
              >
                <span className="font-mono text-[0.5rem] font-semibold text-card-foreground/45 sm:text-[0.65rem]">
                  {formatCardNumber(highlightIndex + 1)}
                </span>
                <span className="text-[0.55rem] font-semibold uppercase leading-[1.3] tracking-[0.06em] sm:text-[0.68rem] sm:leading-relaxed lg:text-xs">
                  {highlight}
                </span>
              </li>
            ))}
